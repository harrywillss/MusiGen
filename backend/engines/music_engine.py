...
Two implementations:
  * ComfyEngine  — submits a workflow to a running ComfyUI instance and
                    downloads the resulting .wav
  * StubEngine   — synthesises a placeholder tone locally so the UI works
                    end-to-end even without ComfyUI or the music model
...
class StubEngine:
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "available": True,
            "note": (
                "Stub engine — generates a placeholder chord tone. Install "
                "ComfyUI and the MiniMax music model to switch to real "
                "generation."
            ),
        }
...
def select_engine() -> Any:
    ...
    if kind == "comfy":
        return ComfyEngine()
    # auto: use comfy if reachable AND all model files present, else stub
    comfy = ComfyEngine()
    st = comfy.status()
    if st["available"]:
        return comfy
